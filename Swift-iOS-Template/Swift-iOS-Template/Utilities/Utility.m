...
+ (NSString *)getStatusStringFrom:(int)status
{
    NSString *STATUSString = @"";
    switch (status) {
        case -1:
            STATUSString = @"Tạm ngưng";
            break;
        case 0:
            STATUSString = @"Chờ duyệt";
            break;
        case 1:
            STATUSString = @"Đã duyệt";
            break;
        case 2:
            STATUSString = @"Đơn hàng từ chối";
            break;
        case 3:
            STATUSString = @"Dự thảo";
            break;
        default:
            STATUSString = @"Tất cả";
            break;
...
+(NSString*)createFolderwithParam:(NSString*)shopCode :(NSString*)type {
    
    NSString *filePath = [NSString stringWithFormat:@"/%@/%@/%@/%@",[Utility formatDateTime:[NSDate date],@"yyyy"],[Utility formatDateTime:[NSDate date],@"M"],shopCode,type];
//    DLogInfo(@"Path = %@", filePath);
    return filePath;
}

+ (BOOL)createFolderWithPath:(NSString *)filePath {
    NSArray *directoryNames = [filePath componentsSeparatedByString:@"/"];
    ...
    for (int i = 0; i < [directoryNames count] ; i++) {
        if (i == 0) {
            folderPath = [documentsDirectory stringByAppendingPathComponent:[directoryNames objectAtIndex:i]];
        } else {
            folderPath = [folderPath stringByAppendingPathComponent:[directoryNames objectAtIndex:i]];
        }
...
+(NSString*)createImageFileName:(NSString*)shopCode : (NSString*)customerCode :(NSString*) mediaObjectType : (NSString*) staffCode : (NSDate*) currentDate : (NSString*) fileType{
    if (!customerCode) {
        customerCode = @"";
    }
    NSString *fileName = [NSString stringWithFormat:@"%@%@_%@_%@_%@_%@%@",shopCode,customerCode,mediaObjectType,staffCode,[Utility formatDateTime:currentDate,DATE_FORMAT_NOW_WITHOUT_SPACE],[Utility formatDateTime:currentDate,DATE_FORMAT_NOW_WITHOUT_SPACE],fileType];
    return fileName;
}
...
+ (NSString *) formatDateTime:(NSDate *)date, ...{
    ...
    va_start(args, date);
    id arg = nil;
    NSString *format = DATE_FORMAT_NOW;
...
+ (NSString *)convertToDateTimeWith:(NSDate *)date format:(NSString*)format
{
    NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
    [dateFormatter setDateFormat:format];
    return [dateFormatter stringFromDate:date];
...
+(NSString *)dateFormat:(NSString *)dateInput convertFrom:(NSString *)convertFrom to:(NSString *)to
{
    NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
    [dateFormatter setDateFormat:convertFrom];
    NSDate *date = [dateFormatter dateFromString:dateInput];
    [dateFormatter setDateFormat:to];
    [dateFormatter stringFromDate:date];
    NSString *dateText = [NSString stringWithFormat:@"%@",[dateFormatter stringFromDate:date]];
    return dateText;
}

+(NSString *)formatCurrency:(NSString *)currencyInput{
    long long num = [currencyInput longLongValue];
...
+ (NSString*)roundDecimal:(NSString*)num
{
    ...
    return [NSString stringWithFormat:@"%@", [numF stringFromNumber:[NSNumber numberWithDouble:[num doubleValue]]]];
}
@end
